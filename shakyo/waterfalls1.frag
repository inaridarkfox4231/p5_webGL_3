...
vec3 GetCameraRayDir( const in vec2 vWindow, inout vec3 vCameraPos, const in vec3 vCameraTarget, const in float fFov )
{
	...
	float fYpos = r * r - clamp(mod(iTime * radius * 2.0, 1.2) - 0.2, 0.0, 1.0);
...
{
	float f = SmoothNoise( vPos * vec3(16.0, 2.0, 16.0) + vec3(0.0, mod(iTime * 20.0, 1000.0), 0.0) );
	return f * f;
}
...
vec3 GetSceneColour( const in vec3 vRayOrigin,  const in vec3 vRayDir )
{
	...
	if(fWaterT < vResult.w)
	{
		...
		vWaterUV *= 3.0;
		vec2 vWaterSpeed = vec2(0.0, iTime) * 4.0;
...
{
	gPixelRand = hash(fract(iTime) + fragCoord.x + fragCoord.y * 1009.0);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	//float fExposure = 3.0;
	float fFov = 0.5;
	float fExposure = clamp(iTime * 0.5, 0.0, 5.0);
